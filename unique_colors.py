...
def newcolor(currentlist):
    ...
    for i in range(len(currentlist)):
        hsvcolors.append(colorsys.rgb_to_hsv(currentlist[i][0],currentlist[i][1],
        currentlist[i][2]))
    ...
    while np.size(np.transpose(n.nonzero()))>= len(edges)-1 or len(mlab.find(n>1))>0:
        i = i+1
        (n,edges)=np.histogram(angles,i,(0,360))
    valid = mlab.find(n == 0)
    D=np.zeros(len(angles))
    for i in range(len(valid)):
        new_angle = circ_mean([edges[valid[i]],edges[valid[i]+1]], radians = False)
